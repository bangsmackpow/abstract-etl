...
function generateV7Markdown(f) {
  ...
  if (chain.length === 0) {
    ...
    chain.forEach((e, i) => {
      ...
      md += `**GRANTOR(S):** ${(e.grantors || []).map((g) => val(g)).join(', ') || 'ΓÇö'}\n`;
      md += `**GRANTEE(S):** ${(e.grantees || []).map((g) => val(g)).join(', ') || 'ΓÇö'}\n`;
  ...
  if (mortgages.length === 0) {
    md += 'NONE ΓÇö NO OPEN MORTGAGE OR DEED OF TRUST WAS INCLUDED OR CLEARLY IDENTIFIED IN THE PROVIDED DOCUMENTS.\n\n';
  } else {
    mortgages.forEach((m, i) => {
      md += `**(${i + 1}) ${dash(m.document_title)}**\n`;
      md += `**BORROWER(S):** ${(m.borrowers || []).map((b) => val(b)).join(', ') || 'ΓÇö'}\n`;
  ...
  if (liens.length === 0) {
    md += "NONE ΓÇö NO OPEN JUDGMENT, LIEN, UCC, STATE TAX LIEN, FEDERAL TAX LIEN, MECHANIC'S LIEN, LIS PENDENS, BANKRUPTCY, OR FORECLOSURE DOCUMENT WAS INCLUDED OR CLEARLY IDENTIFIED IN THE PROVIDED DOCUMENTS.\n\n";
  } else {
    liens.forEach((l, i) => {
