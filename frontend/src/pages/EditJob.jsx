...
export default function EditJob() {
  ...
  const handleDownloadDocx = async () => {
    ...
    } catch { setError('Word download failed.'); }
  ...
  const handleDownloadMd = async () => {
    ...
    } catch { setError('Markdown download failed.'); }
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div className="flex gap-3 mb-6">
        ...
        <button className="btn btn-ghost" onClick={handleDownloadMd} disabled={downloadingMd} style={{ border: '1px solid #ddd' }}>
          {downloadingMd ? 'Generating...' : '⬇ Download Markdown (.md)'}
        </button>
      </div>

      <div className="alert alert-info text-sm mb-6">
        🤖 <strong>AI-extracted</strong> fields are marked. You can choose <strong>Alternatives ▼</strong> if the detection was ambiguous.
      </div>
      ...
      {/* NOTES */}
      <div style={{ marginBottom: 12, fontWeight: 'bold', fontSize: 14, color: '#1a365d' }}>ABSTRACTOR NOTES (INTERNAL ONLY)</div>
      <div className="card mb-6">
        <div className="card-body">
