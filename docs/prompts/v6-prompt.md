...
- Build a PDF DOCUMENT ACCOUNTING listing identifying the document shown on every page or page range. Place this in both the document_accounting schema field and within Additional Information.
...
- Township should normally be the city from the property address.
...
- Capture the borrower, property address, county, city, parcel ID, account number, current vesting owner, assessor owner, legal or assessor description, acreage, and assessment information when available.
...
- If the requested property is not vested as ordered, or the address or parcel does not match, identify the discrepancy in notes.

### TAX INFORMATION RULES

- Capture tax information exactly as shown.
- Include all installments, original bill amounts, due dates, payment dates, payment amounts, balances, penalties, interest, land value, improvement value, total value, and actual delinquent amounts.
- Delinquent tax is not a yes/no field. Show the actual amount owed using the tax_delinquent object (original_amount, due_date, full_delinquent_amount).
- Do not state that taxes are paid unless the source clearly shows payment.
- If current-year or next-year tax information requested by the order is not provided, state that it was not provided.
...
- For each entry, capture when available: DOCUMENT TITLE, GRANTOR(S), GRANTEE(S), DATED DATE, RECORDED DATE, BOOK/PAGE OR INSTRUMENT NUMBER, CONSIDERATION, IN/OUT SALE, NOTES.
...
- Use Additional Information for shorthand references when full indexing is unnecessary.
...
- If a will or estate document affects title, identify when available: DECEDENT, DATE OF DEATH, WILL DATE, RECORDING REFERENCE, DEVISEE(S), HEIRS, RELEVANT NOTES.
- Supporting will, probate, or fiduciary references may be shown as short starred entries in Additional Information, for example: *WILL OF JOSEPH MICHAEL CASSELL 180000213, *FIDUCIARY RECORDS FOR JOSEPH NEWTON CASSELL 010000039.
...
- Capture when available: DOCUMENT TITLE, BORROWER(S), LENDER, TRUSTEE, BENEFICIARY / NOMINEE, DATED DATE, RECORDED DATE, BOOK/PAGE OR INSTRUMENT NUMBER, AMOUNT, MATURITY, LOAN NUMBER, MIN, OPEN / CLOSED ENDED, STATUS, NOTES, ASSOCIATED DOCUMENTS.
- Place assignments, substitute trustees, modifications, subordinations, riders, and related documents under the mortgage they relate to whenever possible.
- Clearly identify each related document by document type, dated date, recorded date, recording reference, parties, and material terms.
- Do not state that a mortgage is released unless a release, satisfaction, or foreclosure clearly supports that conclusion.
- If no open mortgage is found but the order requests the last mortgage and release, include the pertinent mortgage and release when provided.

### JUDGMENTS / LIENS RULES

- Index each judgment, lien, UCC, tax lien, lis pendens, or other involuntary encumbrance separately.
- Capture when available: DOCUMENT TITLE, PLAINTIFF OR LIENHOLDER, DEFENDANT OR DEBTOR, CASE NUMBER, DATE OF JUDGMENT OR LIEN, RECORDED DATE, BOOK/PAGE OR INSTRUMENT NUMBER, AMOUNT, INTEREST, COSTS, ATTORNEY'S FEES, STATUS, NOTES.
...
- Use this section for easements, rights of way, plats, surveys, restrictions, C&Rs, leases, utility documents, outsales requiring detail, probate support documents, agreements, foreclosure support documents, private-road agreements, and other recorded instruments affecting the property.
- Index each actual miscellaneous instrument separately.
- Capture when available: DOCUMENT TITLE, GRANTOR / ASSIGNOR, GRANTEE / ASSIGNEE, DATED DATE, RECORDED DATE, BOOK/PAGE OR INSTRUMENT NUMBER, CONSIDERATION, AREA OR WIDTH, NOTES.
...
- Use Additional Information for shorthand references when full indexing is unnecessary.
- Common shorthand labels include OUTSALE, EASEMENT, UTILITY ROW, C&R, LEASE, PLAT, SOURCE DEED, FORECLOSED DOT, WILL, LOH, and similar labels.
...
- Include PDF DOCUMENT ACCOUNTING covering every page or page range. Use format: "PAGE(S) X-Y: DOCUMENT LABEL" for each entry.
...
- Every related mortgage document is placed under the correct mortgage whenever possible.
...
- All material conflicts and missing-information limitations are clearly identified.
...
4. **TAX INFORMATION**: Capture EVERY installment exactly as shown. Delinquent is NOT a yes/no field — capture original amount, due date, and full delinquent amount shown on the ticket including penalties and fees.
...
See `docs/schemas/v6-schema.json` for the full JSON schema.
