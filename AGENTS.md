...
## Core Services
- `googleAiService.js`: **Primary AI Engine.** Handles native PDF pass-through for v1 (Legacy), v2 (ProTitleUSA), v4 (Hazelwood), and v5 (Standard) schemas. Uses Gemini 2.5 Flash with structured JSON output. Includes robust JSON sanitization with brace-depth tracking and fallback parsing. V5 schema follows clean format with 8 sections and ALL CAPS where practical.
...
- `v5PdfGenerator.js`: Builds clean-format PDF reports for v5 jobs. 8 sections, standard fonts (Helvetica, Times-Roman), no Hazelwood branding.
- `docxGenerator.js`: Builds .docx files for v1, v2, and v5 jobs. Routes to schema-specific generators via `templateVersion` parameter. V5 DOCX uses Arial font with 8-section table layout.
- `markdownGenerator.js`: Builds .md files for v1, v2, v4, and v5 jobs. Routes to schema-specific generators via `templateVersion` parameter.
...
6. **templateVersion**: Jobs persist `templateVersion` in the database. Current values: `v1` (Legacy), `v2` (ProTitleUSA), `v4` (Hazelwood), `v5` (Standard). All generators (PDF, DOCX, MD) must receive and route on `templateVersion`. V5 frontend uses `AbstractForm` with `V5Form` component.
