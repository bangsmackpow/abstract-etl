...
- `emailService.js` — transactional email via **Resend** (mandatory provider, no SMTP fallback). Sends from `Abstract ETL <abstract@builtnetworks.com>` (`MAIL_FROM` / `mail_from` setting override). API key from `RESEND_API_KEY` / `resend_api_key` setting. Uses `fetch` (Web API).
- `backupService.js` — SQLite backups via `sqlite.backup()`; DB `settings` table overrides env at runtime (`backup_enabled`, `backup_interval_minutes`, `backup_retention_days`).
...
Required at startup: `JWT_SECRET` (min 10 chars), `ADMIN_EMAIL`, `ADMIN_PASSWORD` (min 8), `GOOGLE_AI_API_KEY`. Optional: `PORT` (3001), `APP_URL`, `RESEND_API_KEY`, `MAIL_FROM`, `DOCS_DIR`, `DB_PATH`, `LOG_LEVEL`, `EXTRACTION_CONCURRENCY`, `STRIPE_*`. See `.env.example`. `stack.env` (tracked) must contain at least `ADMIN_EMAIL`, `ADMIN_PASSWORD`, `GOOGLE_AI_API_KEY` (placeholders only — CI's `env-audit` warns on real secrets). **All runtime config must come from `env.js` (`const { env } = require('../env')`) — never `process.env.X || '<fallback>'` (a hardcoded JWT fallback is an auth bypass).**
...
17. **Move job across tenants (platform)**: `POST /api/platform/jobs/:id/move {toTenantId}` (platform admin only) moves a job, reassigns `createdBy` to the destination tenant's first admin, and writes an `audit_log` entry (`job.move`). `GET /api/platform/tenants/:id/jobs` lists a tenant's jobs; `GET /api/platform/audit` lists recent privileged actions. `PATCH /api/platform/tenants/:id/admin-email {email}` updates a tenant admin user's login email (platform admin only; writes `audit_log` `tenant.admin_email`).
18. **Auth (Track 2)**: custom auth (keep — do NOT migrate to better-auth). Email-OTP MFA (`POST /auth/verify-otp` after login returns `needsMfa`; enable/disable at `/auth/mfa/*`), one-time password reset (`/auth/forgot-password` → `/auth/reset-password`, SHA-256-hashed tokens in `password_reset_tokens`, 1h expiry), self-serve change password (`PATCH /auth/password`). OTP codes are SHA-256 hashed in `users.otp_code_hash`. **`comparePassword` is SYNC (bcrypt.compareSync) — never `await`-less Promise checks (they silently pass and bypass auth).** **Token revocation**: JWTs carry `tv` (token_version); password change/reset bumps `users.token_version` → older tokens get 401. `requireAuth` enforces it. **Auth endpoints are rate-limited** (`middleware/rateLimit.js`: login 10/15min per ip+email, OTP 5/10min, signup 5/hr, forgot-password 5/15min; `trust proxy` set for real client IPs behind Cloudflare→NPM).
19. **Per-tenant settings (Track 1a)**: `tenant_settings` key-value table scoped by tenant. Tenant admin: `GET/PATCH /api/admin/settings`. Platform admin: `GET/PATCH /api/platform/tenants/:id/settings`. Global system settings (Resend email + backups) at `GET/PATCH /api/admin/system/settings` (platform-only). Keys: `notification_email`, `daily_report_enabled`, `daily_report_time`, `default_output_format`, `enable_completion_emails`, `enable_bulk_import_emails`. The key allowlist is enforced centrally in `tenantRepo.setTenantSettings` (`TENANT_SETTING_KEYS`) — unknown keys are silently dropped, no caller (admin or platform route) may bypass it.
...
23. **Internal IP is gated**: `/api/docs/*` (rules, Gemini prompts, schemas) is **platform-admin only** (`requireAuth, requirePlatformAdmin` in `routes/docs.js`); the frontend Docs page and nav link are also `platformOnly`. Never relax this — the prompts are proprietary. Any future IP-bearing route follows the same pattern.
24. **Admin seeding is one-time**: `seedAdmin` (in `index.js`) creates the platform admin **only if the user doesn't exist**. It never overwrites the stored password on boot (a changed/self-service password is preserved; a differing `ADMIN_PASSWORD` env only logs a warning). Recovery path is the forgot-password email flow, not env.
...
- **Admin seeding is one-time**: `seedAdmin` never resets an existing admin's password. If you're locked out, use the forgot-password email flow or delete the row in `users` and restart — do NOT change `ADMIN_PASSWORD` expecting a boot-time reset.
...
- **Pushing to `main` deploys to prod**: `build.yml` runs on every main push (validate → build/push ghcr.io images → trigger Portainer webhook) and also runs validate+build (no push/deploy) on PRs. On main, after the webhook fires it polls `https://abstract.builtnetworks.com/api/health` for up to 5 min. A `notify` job emails `ADMIN_EMAIL` on failure (needs `RESEND_API_KEY` + `MAIL_FROM` + `ADMIN_EMAIL` secrets). There is no PR gate on the build workflow. CI installs with `--legacy-peer-deps`.
