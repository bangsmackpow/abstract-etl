...
- **✅ Shipped (2026-09-02):** Email-OTP MFA, password reset (one-time email link), self-serve change password, per-tenant settings (tenant admin + platform editor), report `.zip` export (DOCX+PDF+MD, date range, 200 cap), daily usage report email, better-sqlite pragmas + in-process extraction queue, marketing landing page + public 7-day-trial signup (`/` → landing, app under `/app`), Stripe billing (test mode) with per-tenant tiers (Solo/Team/Enterprise). Transactional email moved to **Resend** (mandatory provider; sends from `abstract@builtnetworks.com`; no SMTP fallback). Platform admin can update a tenant admin's login email (`PATCH /api/platform/tenants/:id/admin-email`).
- **✅ Security hardening (2026-09-02):** `/api/docs/*` is platform-admin only (prompts/schemas are internal IP); removed the hardcoded `JWT_SECRET` fallback and all `process.env` reads now go through zod-validated `env.js`; `express-rate-limit` on all auth endpoints (login/OTP/signup/forgot-password, keyed by ip+account) with `trust proxy` for real client IPs; JWT token revocation via `users.token_version` (`tv` claim bumped on password change/reset); **fixed a silent auth bypass — `comparePassword` is now sync and always awaited (login previously never checked the password)**; tenant-settings key allowlist enforced centrally in `tenantRepo.setTenantSettings`; platform admin-email update no longer returns password/OTP hashes; tenant settings now honored for completion/bulk-import emails (and bulk notification no longer emails the platform `ADMIN_EMAIL`); HTML-escaped all user/PDF-derived email content and fixed email links to `/app/jobs/*`; `seedAdmin` now seeds the platform admin once and never resets the stored password on boot.
